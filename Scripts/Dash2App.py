...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            html.H5("Filtros"),
            # Selección múltiple en todos los dropdowns
            dcc.Dropdown(
                id="filtro_clinic",
...
def actualizar_dashboard(clinic, department, admit, diagnostico, start_date, end_date,
    ...
    # Filtros (soportan listas gracias a multi=True)
    if clinic:
        dff = dff[dff["Clinic Name"].isin(clinic)]
    if department:
        dff = dff[dff["Department"].isin(department)]
    if admit:
        dff = dff[dff["Admit Type"].isin(admit)]
    if diagnostico:
        dff = dff[dff["Diagnosis Primary"].isin(diagnostico)]
    if clinica_click:
    ...
    if not df_mes.empty:
        ...
        fig_pacientes_mes = px.bar(title="Sin datos")
    ...
    fig_series = px.line(df_series, x="Appt Start Time", y="Pacientes",
                         title="Pacientes por Fecha")
    ...
    if not dff.empty:
        ...
        fig_dia = px.bar(title="Sin datos")
    ...
    if not dff.empty:
        ...
        fig_hora = px.bar(title="Sin datos")
    ...
    # >>> Devolver TODOS los outputs esperados
    return (
        f"{espera_prom:.1f} min", color_espera,
